...
for i, PSR in enumerate(PSR_list[0:1]):
    ...
    ec_timing_model = models.get_model(
        glob.glob(f"./data/NG_15yr_dataset/par/{PSR}*.nb.par")[0])  # Ecliptical coordiantes
    eq_timing_model = ec_timing_model.as_ICRS(
        epoch=Time(data.loc[PSR, "POSEPOCH"], format="mjd"))  # Equatorial coordinates
    ...
    plt.suptitle(PSR)
    plt.tight_layout()
    plt.savefig("./figures/density_plots/" + PSR + "_PM.png", bbox_inches='tight')
    plt.show()
    ...
    # VLBI
    VLBI_x0: float = data.loc[PSR, "VLBI_PX"]
    VLBI_uL: float = data.loc[PSR, "VLBI_PX_uL"]
    VLBI_uR: float = data.loc[PSR, "VLBI_PX_uR"]
    ...
    fig.write_image(f"./figures/density_plots/{PSR}_PX.png")
    fig.show()
